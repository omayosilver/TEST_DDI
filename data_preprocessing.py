...
df_drugs_smiles = pd.read_csv('DATA_CSV\Drugbank_CID_pubchem_drug_SMILES.csv')
...
drug_id_mol_graph_tup = [(id, Chem.MolFromSmiles(smiles.strip())) for id, smiles in
                         zip(df_drugs_smiles['drug_id'], df_drugs_smiles['drug_smile'])]
...
#print(TOTAL_ATOM_FEATS)

##### DDI statistics and counting #######
df_all_pos_ddi = pd.read_csv('DATA_CSV\AdverseDrugDrugInteractionDataset.csv')
all_pos_tup = [(h, t, r) for h, t, r in zip(df_all_pos_ddi['drug1_id'], df_all_pos_ddi['drug2_id'], df_all_pos_ddi['drug_drug_interaction_id'])]
